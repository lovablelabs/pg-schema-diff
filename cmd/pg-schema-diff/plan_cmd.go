...
func createPlanOptionsFlags(cmd *cobra.Command) *planOptionsFlags {
	...
	cmd.Flags().BoolVar(&flags.noPrivileges, "no-privileges", false, "If set, will skip generating GRANT and REVOKE statements in the migration plan.")
...
func parsePlanOptions(p planOptionsFlags) (planOptions, error) {
	...
	if p.noPrivileges {
		opts = append(opts, diff.WithNoPrivileges())
	}
